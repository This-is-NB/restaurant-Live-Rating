...
const FeaturedProducts: React.FC<FeaturedProductsProps> = ({
  ...
  const handleSeeReviews = useCallback((product: Product) => {
    setModalProduct(product);
    setShowReviewsModal(true);
  }, []);

  const handleLikeClick = useCallback((productId: number) => {
    setAnimating(a => ({ ...a, [productId]: 'like' }));
    onLike(productId);
    setTimeout(() => setAnimating(a => ({ ...a, [productId]: null })), 700);
  }, [onLike]);

  const handleDislikeClick = useCallback((productId: number) => {
    setAnimating(a => ({ ...a, [productId]: 'dislike' }));
    onDislike(productId);
    setTimeout(() => setAnimating(a => ({ ...a, [productId]: null })), 700);
  }, [onDislike]);

  const openVariantModal = useCallback((productId: number, categoryId: number) => {
    // Placeholder for variant modal logic
  ...
  const filteredReviews = useMemo(() => {
    ...
    return chatMessages.filter(
      m => m.items.some(item => item.toLowerCase().includes(modalProduct.name.toLowerCase()))
    );
  }, [chatMessages, modalProduct]);
                  ...
                  <div className="item-card-placeholder-detail-new item-without-img-detail">
                      ...
                      <div className="cart-new-btn rating-tp">
                        {!cart[product.id]?.quantity ? (
                          ...
                          <div className="quantity-btn" id={`div-${product.id}`}>
                            ...
                            ></div>
                            <input type="hidden" value="update" className="1" />
                            <input
                              type="text"
                    ...
                    <div className="vote-section mt-2" style={{ position: 'relative', minHeight: 32 }}>
                      <span
                        ...
                        title="Like"
                      >
                        <i className="fa fa-thumbs-up"></i> {votes[product.id]?.likes || 0}
                      </span>
                      <span
                        ...
                        title="Dislike"
                      >
                        <i className="fa fa-thumbs-down"></i> {votes[product.id]?.dislikes || 0}
                      </span>
